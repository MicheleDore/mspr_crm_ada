...
@Repository
    ...
    @Query(value = "SELECT  SUM(p.Prix * cc.Quantite) AS montantTotal FROM commandes c JOIN contenucommandes cc ON cc.ID_Commande = c.ID_Commande JOIN produits p ON p.ID_Produit = cc.ID_Produit GROUP BY c.ID_Commande ORDER BY c.ID_Commande",nativeQuery = true)
    public List<Integer> findListeCommandeValeur();
    ...
    public int findContenuCommande(@Param("id")int id);
